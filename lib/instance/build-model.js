...
module.exports = async function (schema) {
  ...
  await iterateNduts(async function (cfg) {
    ...
    for (const f of files) {
      ...
      const mod = require(f)
      if (_.isPlainObject(mod) && mod.properties) {
        _.forOwn(mod.properties, (v, k) => {
          const ext = k === base ? options.schema : {}
          schema.properties[k] = _.merge(schema.properties[k], v, ext)
        })
      }
  ...
  if (['postgresql', 'mysql', 'sqlite3', 'oracle'].includes(dbName)) {
    _.forOwn(schema.properties, (v, k) => {
      ...
      // object/array
      if (dbName === 'mysql' && (typeof v.type === 'string' && ['object', 'array'].includes(v.type.toLowerCase()) ||
        typeof v.type === 'function' && ['Object', 'Array'].includes(v.type.name)) && !v[dbName]) {
        v[dbName] = {
          dataType: 'LONGTEXT'
        }
      }
