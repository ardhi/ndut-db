module.exports = async function (model) {
  const { _, aneka, fastGlob, importDataFile, getConfig, getNdutConfig } = this.ndut.helper
  const { pascalCase } = aneka
  const config = await getConfig()
  const path = require('path')
  const { schemas } = this.ndutDb

  const fixture = _.map(schemas, s => ({ name: s.alias }))
  await model.DbModel.create(fixture)

  const handler = async (records, opts) => {
    records = _.map(records, r => {
      if (!r.name) r.name = _.startCase(r.value.toLowerCase())
      r.model = opts.model
      return r
    })
    await model.DbLookup.create(records)
  }

  for (let n of config.nduts) {
    n = await getNdutConfig(n)
    const files = await fastGlob(`${n.dir}/ndutDb/lookup/*.{json,jsonl}`)
    const models = []
    _.forOwn(model, (v, k) => {
      if (k.startsWith(_.upperFirst(n.alias))) models.push(k)
    })
    for (const f of files) {
      const modelName = pascalCase(`${n.name === 'app' ? '' : n.alias} ${path.parse(f).name}`)
      if (!models.includes(modelName)) continue
      await importDataFile(f, handler, { handler: { model: modelName } })
    }
  }
}
