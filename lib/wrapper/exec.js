module.exports = async function (name, method, request, ...args) {
  ...
  else if (['remove', 'create'].includes(method)) result = await model[method](args[0], args[1])
  else if (['update'].includes(method)) result = await model[method](args[0], args[1], args[2])
  else result = await model[method](...args)
  */
  let result = await model[method](...args)
  if (this.ndutDb.interceptor[method]) {
    for (const obj of this.ndutDb.interceptor[method]) {
